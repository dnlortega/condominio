...
import { handleResidentLogin } from '@/app/actions/auth';
...
export default function ResidentLoginPage() {
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const success = await handleResidentLogin(formData);
                ...
                <div className="bg-zinc-900/50 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 md:p-10 shadow-2xl shadow-black/50 relative overflow-hidden group">
                    ...
                    <form onSubmit={onSubmit} className="space-y-6 relative z-10 pb-4">
                        <div className="space-y-2">
                            <Label htmlFor="email" className="text-xs uppercase tracking-widest text-zinc-500 font-bold ml-1">E-mail</Label>
                            <Input
                                id="email"
                                name="email"
                                type="email"
                                ...
                                required
                                className="h-14 bg-black/40 border-white/10 text-white placeholder:text-white/20 rounded-2xl px-5 focus-visible:ring-1 focus-visible:ring-white/30 focus-visible:border-white/30 transition-all w-full"
                            />
                        </div>

                        <div className="space-y-2">
                            <Label htmlFor="password" className="text-xs uppercase tracking-widest text-zinc-500 font-bold ml-1">Senha</Label>
                        ...
                        <p className="text-xs text-zinc-500 text-center">
                            Ainda não tem acesso? Fale com a síndica para cadastrar sua unidade.
                        </p>
                    </form>
                </div>
            </motion.div>
