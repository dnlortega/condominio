...
import { handleResidentLogout } from "@/app/actions/auth";

const routes = [
    { label: "Início", icon: LayoutDashboard, href: "/portal" },
    { label: "Avisos", icon: Megaphone, href: "/portal/avisos" },
...
export function PortalSidebar({ user }: { user: ResidentInfo }) {
    ...
    return (
        <div className="space-y-4 py-8 flex flex-col h-full bg-white text-slate-600 border-r border-slate-100 relative overflow-hidden transition-all duration-500">
            ...
            <div className="px-8 py-2 flex-1 relative z-10">
                <Link href="/portal" className="flex items-center group mb-10">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[9px] uppercase tracking-[0.4em] text-primary/70 mt-1.5 font-bold">Portal</span>
                    </div>
                </Link>

                <div className="mb-8 px-4 py-3 rounded-2xl bg-slate-50 border border-slate-100">
                    <p className="text-sm font-semibold text-slate-800 truncate">{user.name}</p>
                    {user.apartment && <p className="text-xs text-slate-400">Apto {user.apartment}</p>}
                </div>

                <div className="space-y-1.5">
                    <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-slate-400 mb-6 px-4">Menu</p>
                    {routes.map((route) => {
            ...
            <div className="px-8 py-8 border-t border-slate-50 relative z-10 space-y-3">
                ...
                <form action={handleResidentLogout}>
                    <Button
                        variant="ghost"
...
export function MobilePortalSidebar({ user }: { user: ResidentInfo }) {
    return (
        <Sheet>
